...
async function generateForEmployee(employeeId, type) {
...
export async function GET(req) {
  ...
  for (const id of Object.keys(employees)) {
    try {
      results[id] = await generateForEmployee(id, type)
    } catch (err) {
      results[id] = `error: ${err.message}`
    }
  }

  return Response.json({ ok: true, type, generated: results, at: new Date().toISOString() })
}
